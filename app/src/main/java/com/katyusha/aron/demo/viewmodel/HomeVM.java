...
import com.katyusha.aron.demo.api.HomeApiService;
...
//        }
        ...
        BLRequest.getInstance().createApi(HomeResponse.class).get(Urls.HOME, params.urlParamsMap).subscribe(subscriber);

        // 自定义接口
//        HomeApiService service = BLRequest.getInstance().create(HomeApiService.class);
//        observe(service.getHomeData(params.urlParamsMap)).subscribe(subscriber);
